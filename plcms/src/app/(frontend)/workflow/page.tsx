...
export default async function CreateWorkflowPage() {
  ...
  const cols = await payload.collections
  console.log(cols)
  ...
  // Fetch available collections from Payload CMS
  useEffect(() => {
    const fetchCollections = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter out system collections like users, workflows, etc.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        router.push('/admin/collections/workflows')
      } else {
